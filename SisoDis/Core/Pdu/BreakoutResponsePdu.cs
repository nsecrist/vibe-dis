...
public record struct BreakoutResponsePdu(
    EntityId RespondingEntityId,
    uint RequestId,
    ushort BreakoutResponseStatus,
...
{
    public const ushort PdTypeValue = 47;
    ...
    {
        BinaryPrimitives.WriteUInt16BigEndian(buffer.Slice(offset, 2), (ushort)RespondingEntityId.Value);
        offset += 2;

        BinaryPrimitives.WriteUInt32BigEndian(buffer.Slice(offset, 4), RequestId);
        offset += 4;

        BinaryPrimitives.WriteUInt16BigEndian(buffer.Slice(offset, 2), BreakoutResponseStatus);
        offset += 2;
    ...
    public static BreakoutResponsePdu Deserialize(ReadOnlySpan<byte> buffer, int offset = 0)
    {
        ...
        ushort respondingEntityIdValue = BinaryPrimitives.ReadUInt16BigEndian(buffer.Slice(pos, 2));
        EntityId respondingEntityId = new EntityId(respondingEntityIdValue);
        pos += 2;

        uint requestId = BinaryPrimitives.ReadUInt32BigEndian(buffer.Slice(pos, 4));
        pos += 4;

        ushort breakoutResponseStatus = BinaryPrimitives.ReadUInt16BigEndian(buffer.Slice(pos, 2));
        pos += 2;
        ...
        return new BreakoutResponsePdu(
            respondingEntityId,
            requestId,
            breakoutResponseStatus,
            numberOfFixedDatum,
    ...
    {
        private EntityId _respondingEntityId = new(0);
        private uint _requestId = 0;
        private ushort _breakoutResponseStatus = 0;
        ...
        public Builder WithRespondingEntityId(EntityId id) { _respondingEntityId = id; return this; }
        public Builder WithRequestId(uint id) { _requestId = id; return this; }
        public Builder WithBreakoutResponseStatus(ushort status) { _breakoutResponseStatus = status; return this; }
        ...
        public BreakoutResponsePdu Build() => new(
            _respondingEntityId,
            _requestId,
            _breakoutResponseStatus,
            _numberOfFixedDatum,
